...
from sqlalchemy import text, MetaData, Table, Column, Integer, String, ForeignKey, insert, select, bindparam, \
    literal_column, and_, or_, func, asc, desc
...
"""
Labeled Expression & Textual Column Expression
"""
...
select_stm = select(User.name, Address.user_id, Address.email_address).where(User.id == Address.user_id,
                                                                             User.name == 'sandy', Address.id > 2)
select_and_or = select(User.name, Address.user_id, Address.email_address).where(
    and_(or_(User.name == 'sandy', User.name == 'dariush'),
         and_(User.id == Address.user_id, Address.email_address.contains('pars'))))
...
"""
Join Clause & Explicit From Clause
"""

"""
ON clause is inferred automatically from foreign keys
"""
...
select_stm = select(func.count("*")).select_from(address_table)

"""
Setting the ON Clause
"""

select_stm = select(user_table.c.name, address_table.c.email_address).select_from(user_table).join(address_table,
                                                                                                   user_table.c.id == address_table.c.user_id)

"""
OUTER and FULL join
"""
select_stm1 = select(user_table, address_table).join(address_table, isouter=True)
select_stm2 = select(address_table, user_table).outerjoin(user_table)
select_stm = select_stm1.union_all(select_stm2)
...
Order By
"""
select_stm = select(user_table).order_by(user_table.c.name.desc())

"""
ORM Order By
"""

select_stm = select(User).order_by(User.id.desc())
...
COUNT Function
"""
count_func = func.count(user_table.c.id)
print(count_func)
...
count_func = func.count(address_table.c.id)
select_stm = select(user_table.c.name, count_func.label("emails")).join(
    address_table, user_table.c.id == address_table.c.user_id).group_by(user_table.c.id).having(count_func > 1)
...
select_stm = select(User.name, count_func.label('email_count')).join(Address).group_by(User.id).having(count_func > 1)

"""
Ordering or Grouping by a Label
"""

select_stm = select_stm.order_by(asc('email_count'))
...
alias1 = user_table.alias()
alias2 = user_table.alias()

select_stm = select(alias1.c.id, alias1.c.name, alias2.c.name, alias2.c.id).join(alias2, alias1.c.id > alias2.c.id)
...
select_stm = select(alias1.fullname, alias2.email_address).join(alias2, alias1.id == alias2.user_id).where(alias2.email_address.contains('pars'))
...
subquery1 = select(address_table.c.user_id, func.count(address_table.c.id).label('email_count')).group_by(address_table.c.user_id).subquery()
select_stm = select(user_table.c.name, user_table.c.fullname, subquery1.c.email_count).join_from(user_table, subquery1)
